...
func (task *Task) process() string {
	...
	time.Sleep(delay)
	return fmt.Sprintf(" %s = Processed data for task %d.", task.Data, task.ID)
	}

// Worker tasks
func worker(tasks <-chan Task, results chan<- Result, wg *sync.WaitGroup) {
	defer wg.Done()
	log.Printf("Worker %d started", getgoRoutineID())
	for task := range tasks {
		processingResult := task.process()
		result := Result{TaskID: task.ID, ResultData: processingResult}
		results <- result
		log.Printf("Worker %d processed task %d", getgoRoutineID(), task.ID)
	}
	log.Printf("Worker %d finished", getgoRoutineID())
}
...
func main() {
	...
	for i := 0; i < numTasks; i++ {
		...
		tasks <- task
		log.Printf("Task %d added to task list", i)
	}
	...
	log.Printf("Started collecting results to file...")
	...
	log.Printf("All tasks processed and results written to file.")
}